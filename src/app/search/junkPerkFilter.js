...
function initJunkPerks(stores, dupeReport) {
  if (_junkPerkMaps === null && stores.length > 0) {
    ...
    console.log('_junkPerkMaps', _junkPerkMaps);
    ...
    _.each(_junkPerkMaps.legendaryArmor, (item) => {
      ...
      const armorPerks = item.sockets.sockets
        .filter((socket) => {
          return (
            socket.plugOptions.filter((options) => {
              return options.plugItem.itemTypeAndTierDisplayName == 'Legendary Armor Perk';
            }).length > 0
          );
        })
        .map((socket) => {
      ...
      if (item.id == '6917529081156579843') {
        console.log('armorPerks', armorPerks, item.name, item);
      }
...
function junkPerkFilter(item, dupeReport) {
  ...
  if (item.bucket.sort == 'Armor' && item.tier === 'Legendary') {
    ...
    var armorCombos;
    try {
      armorCombos = junkPmByClass.armorCombos[item.id];
    } catch (e) {
      console.log('nothing for this', item.classTypeName, junkPmByClass);
    }
    ...
    const wantedCombos = _.filter(armorCombos, (combo) => {
      ...
      if (fcPerkCount == 1 || scPerkCount == 1) {
        var perkCount = fcPerkCount <= 2 ? fcPerkCount : scPerkCount;
        var perkName = fcPerkCount <= 2 ? fcPerkName : scPerkName;
        var reasonText = perkName + ' Single - ' + perkCount;
        comboReasons.push('Unique Perk ' + (perkCount == 1 ? '' : reasonText));
      ...
      if (!isFourPa && perkPairCount.fivePa >= 2) {
        ...
        /*        if (!(comboString in junkPmByClass.dupeComboStrings)) {
          junkPmByClass.dupeComboStrings[comboString] = [];
        }
        if (junkPmByClass.dupeComboStrings[comboString].indexOf(item.id) == -1) {
          junkPmByClass.dupeComboStrings[comboString].push(item.id);
        }*/
        comboConflicts.push(perkPairCount.fivePa == 2);
        return false;
      }
    ...
    //
    if (item.id == '6917529096052564188') {
    ...
    if (wantedCombos.length == 0) {
      ...
      _.each(armorCombos, (combo, index) => {
        ...
        if (conflicts) {
          ...
          if (junkPmByClass.dupeComboStrings[comboString].length < 2) {
            conflicts = false;
          }
        } else {
          /* Unique Perk */
          const fcPerkName = combo[0];
          const fcPerkCount = junkPmByClass.armorPerkCount[fcPerkName];
          if (!(fcPerkName in junkPmByClass.uniquePerkNames)) {
            junkPmByClass.uniquePerkNames[fcPerkName] = [];
          }
          if (junkPmByClass.uniquePerkNames[fcPerkName].indexOf(item.id) == -1) {
            junkPmByClass.uniquePerkNames[fcPerkName].push(item.id);
          }
          if (junkPmByClass.uniquePerkNames[fcPerkName].length == fcPerkCount) {
            conflicts = true;
            reason = 'Unique Perk Conflict - ' + fcPerkName;
          }
        }
        lines.push({
          combo: combo,
...
export { initJunkPerks, junkPerkFilter, junkPerkPresets as junkPerkConfig };
